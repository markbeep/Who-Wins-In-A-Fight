...
func testCategoryToManyBattles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.Battles().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadBattles(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Battles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Battles = nil
	if err = a.L.LoadBattles(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Battles); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCategoryToManyCards(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CategorySlice{&a}
	if err = a.L.LoadCards(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCategoryToManyUserTokens(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CategorySlice{&a}
	if err = a.L.LoadUserTokens(ctx, tx, false, (*[]*Category)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCategoryToManyAddOpBattles(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.R.Battles[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Battles[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Battles().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testCategoryToManyAddOpCards(t *testing.T) {
	var err error
...
func testCategoryToManyAddOpUserTokens(t *testing.T) {
	var err error
...
var (
	categoryDBTypes = map[string]string{`ID`: `integer`, `Token`: `text`, `Title`: `text`, `Description`: `text`}
	_               = bytes.MinRead
)
